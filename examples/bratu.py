...
h = 1.0 / (N + 1)
_off = jnp.ones(N - 1)
D2 = (jnp.diag(-2.0 * jnp.ones(N)) + jnp.diag(_off, 1) + jnp.diag(_off, -1)) / h ** 2
XS = jnp.linspace(0.0, 1.0, N + 2)        # full grid incl. boundaries


def R(u, lam):
    return D2 @ u + lam * jnp.exp(u)      # = 0  <=>  -u'' = lambda e^u
...
def main():
    ...
    lam = np.asarray(br.p)
    norm = np.max(np.abs(br.x), axis=1)   # ||u||_inf
    ...
    mark_folds(ax, lam_f, np.max(np.abs(br.x[i])))
    ax.annotate(rf"$\lambda^\ast\approx{lam_f:.3f}$", (lam_f, np.max(np.abs(br.x[i]))),
    ...
    ax = axes[1]
    picks = [(np.argmin(np.abs(lam[:i] - 1.0)), STABLE_C, "cool, $\\lambda=1$"),
             (i, ACCENT, rf"fold, $\lambda^\ast$"),
             (len(lam) - 1 - np.argmin(np.abs(lam[::-1] - 1.0)), UNSTABLE_C,
              "hot, $\\lambda=1$")]
    for idx, col, lab in picks:
        u_full = np.concatenate([[0.0], np.asarray(br.x[idx]), [0.0]])
        ax.plot(np.asarray(XS), u_full, "-", color=col, lw=2.2, label=lab)
    ax.set_xlabel(r"$x$")
    ax.set_ylabel(r"$u(x)$")
