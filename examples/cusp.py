...
def R2(x, p, q):
    return jnp.array([x[0] ** 3 - q * x[0] + p])
...
def full_fold_set(q0=0.5, q_max=2.5):
    ...
    x = np.concatenate([up.x[:, 0], thru.x[:, 0]])
    q = np.concatenate([up.q, thru.q])
    p = np.concatenate([up.p, thru.p])
    order = np.argsort(x)
...
def main():
    ...
    err_q = np.max(np.abs(q - 3.0 * x ** 2))
    err_p = np.max(np.abs(p - 2.0 * x ** 3))
    ...
    for q, col in [(0.5, STABLE_C), (1.0, ACCENT), (2.0, UNSTABLE_C)]:
        ...
        br = arclength_continuation(Rq, jnp.array([-np.sqrt(q) - 0.4]), p0=1.6 * q,
                                    ds=0.03, ds_max=0.06, n_steps=800,
                                    p_min=-2.2 * q, p_max=2.2 * q, direction=-1.0)
        ax.plot(br.p, br.x[:, 0], "-", color=col, lw=2.0, label=fr"$q={q:g}$")
        if br.turning_points:
            i = np.array(br.turning_points)
