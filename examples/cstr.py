...
def fold_curve(B0 = 6.0, x0 = 0.21132, Da0 = 0.075403):
    ...
    thru = track_fold(G, np.array([x0]), p0 = Da0, q0 = B0, ds = 0.1, ds_max = 0.3,
                      q_min = 3.9, q_max = 14.0, direction = -1.0)   # -> cusp -> other arm
    up = track_fold(G, np.array([x0]), p0 = Da0, q0 = B0, ds = 0.1, ds_max = 0.5,
                    q_min = 3.9, q_max = 14.0, direction = +1.0)     # B up, x -> 0
    x = onp.concatenate([thru.x[:, 0], up.x[:, 0]])
    B = onp.concatenate([thru.q, up.q])
    Da = onp.concatenate([thru.p, up.p])
    order = onp.argsort(x)
...
def main():
    ...
    br = arclength_continuation(R, np.array([0.03]), p0 = 0.02, ds = 0.008,
                                ds_max = 0.02, n_steps = 900, p_min = 0.0,
                                p_max = 0.20, direction = 1.0)
    folds = []
    for i in br.turning_points[:2]:
        xf, Daf, _, res = refine_fold(R, np.array(br.x[i]), float(br.p[i]))
    ...
    stable = branch_stability(R, list(br.x), list(onp.asarray(br.p)), sign = +1.0)  # x_dot = G

    # -- Part 2: the cusp in the (B, Da) plane ---------------------------
    Bc, Dac, xc = fold_curve()
    i_cusp = int(onp.argmin(Bc))
    print(f"cusp: min B={Bc[i_cusp]:.4f} (ref 4), Da={Dac[i_cusp]:.5f} (ref e^-2={onp.exp(-2):.5f})")
    ...
    ax.plot(4.0, onp.exp(-2), "o", color=ACCENT, ms=9, zorder=5)
    ax.annotate(r"cusp $(4,\,e^{-2})$", (4.0, onp.exp(-2)), textcoords="offset points",
