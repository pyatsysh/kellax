...
pure JAX, so kellax's core dependencies stay numpy + jax. The spectrum
functions (``hessian_spectrum`` / ``smallest_eigenvalue`` / ``morse_index``)
want the ``spectra`` extra: ``pip install kellax[spectra]``. In practice jax
pulls scipy in anyway, which is why this went undeclared until v0.5; that is
jax's business to change, not a guarantee kellax should be resting on.
"""
...
def hessian_spectrum(F, x, k: int = 6, which: str = "SA"):
    ...
    if k_eff >= N - 1:                                  # dense: exact, tiny N
        H = jax.jacfwd(jax.grad(F))(np.asarray(x, dtype = float))
    ...
    A = LinearOperator((N, N),
                       matvec=lambda v: onp.asarray(hvp(np.asarray(v))), dtype = float)
